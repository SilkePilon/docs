...
```bash
curl -H "Authorization: Bearer YOUR_TOKEN" https://jotihunt.nl/api/2.0/subscriptions
```

<Note>
  You'll need to register for a Jotihunt account and request API access to receive your authentication token.
</Note>
...
The API implements rate limiting to ensure fair usage. Each client is limited to 60 requests per minute. If you exceed this limit, you'll receive a `429 Too Many Requests` response.
